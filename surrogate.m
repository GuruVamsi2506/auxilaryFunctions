...
elseif strcmp(method,'PPS') % PPS surrogates
    ...
    for sn=1:N
        ...
        for j=1:dL
            kn=kn+1;
            surr(sn,j)=signal(1,kn);
            sigdist=abs(signal-(signal(kn)+randn*rho));
            [~,kn]=min(sigdist);
            if kn==L
                kn=pl;
            end
    ...
    for sn=1:N
        ...
        for j=1:dL
            ...
            %knold=kn;
            surr(sn,j)=signal(1,kn);
            sigdist=max(abs(signal(:,:)-(signal(:,kn)+randn*rho)*ones(1,L)));
            [~,kn]=min(sigdist);
            if kn==L
                kn=pl;
            end
...
elseif strcmp(method,'MCPP') % (corrected for noise) cycle phase permutation surrogates
...
elseif strcmp(method,'tshift') % cycle phase permutation surrogates
...
elseif strcmp(method,'tshift2') % cycle phase permutation surrogates
